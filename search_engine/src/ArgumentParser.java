...
public class ArgumentParser {
	...
	private void parseArgs(String[] arguments) {
		...
		int i = 0;
		// while index is less than the length of arguments
		while (i < arguments.length) {
			// if argument[i] is a flag
			if (isFlag(arguments[i])) {
				// if at end of the array or arguments[i+1] is a flag instead of
				// value
				...
				// if arguments[i+1] is a value
				else if (isValue(arguments[i + 1])) {
					argumentMap.put(arguments[i], arguments[i + 1]);
					i = i + 2;
				}
			}
			// if is a value without a flag, program will skip to next element.
			else {
				i = i + 1;
			}
		}
	...
	public static boolean isValue(String text) {
		...
		return ((text != null) && (!text.equals("")) && (!text.startsWith("-")));
